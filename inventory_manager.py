...
A comprehensive system designed to facilitate the management
of stock keeping units (SKUs) in an inventory setting.
"""
...
def get_sku():
    ...
    and a unique identifier to create an SKU.

    Returns:
        str: A string representing the SKU
        in the format ItemName-UniqueIdentifier.
    """
    while True:
        try:
            ...
            if item != "" and item.isalnum():
                ...
                print("Invalid Input: Please enter a "
                      "valid alphanumeric item name!")
        except ValueError:
            print("Error: Please try again!")


def get_sku_quantity():
    """ Gets the SKU from the user and then
    prompts for its quantity in stock. Validates the quantity.

    Returns:
        tuple: A tuple containing the SKU as a string
        and its quantity as an integer.
    """
    sku = get_sku()
...
def get_sku_category(sku_categories):
    ...
    while True:
        try:
            selected_index = input(f"Enter the category where your SKU "
                                   f"belongs to (1 - {len(sku_categories)}): ").strip()
...
def inventory_item_class(sku, category, quantity):
    ...
    """
    inventory_item = InventoryItem(sku=sku,
                                   category=category, quantity_in_stock=quantity)
...
def write_inventory_to_file(inventory_item: InventoryItem, file_name):
    ...
    try:
        with open(file_name, "a") as file:
            inventory_string = (f"{inventory_item.sku}, "
                                f"{inventory_item.category}, {inventory_item.quantity_in_stock}")
            file.write(inventory_string)
            file.write("\n")
    except FileNotFoundError:
        print(f"Error: {file_name} was not found!")
    except OSError:
        print(f"Error: Something happened while "
              f"writing to the file: {file_name}")


def summarize_inventory(file_name):
    """Reads inventory item information from a file
    and summarizes the quantity of items by category.

    Parameters:
        file_name (str): The name of the file from which
        inventory information is being read.
    """
    inventory_sum = {}
    try:
...
def list_skus(file_name):
    ...
    Returns:
        list: A list of tuples containing the SKU,
        category, and the quantity in stock.
    """
    sku_list = []
    try:
        with open(file_name, "r") as file:
...
def get_sku_to_remove(sku_list):
    ...
    while True:
        try:
            selected_index = input(f"Select the Stock Keeping Unit (SKU) "
                                   f"that you wish to remove (1 - {len(sku_list)}): ").strip()
            selected_index = int(selected_index)
...
def remove_sku(sku_to_remove, file_name):
    ...
    try:
        ...
        print(f"Error: Something happened while "
              f"modifying the file: {file_name}")
...
def clear_inventory(file_name):
    ...
    Parameters:
        file_name (str): The name of the file from
        which inventory is to be removed.
    """
    try:
...
def main():
    ...
    while True:
        ...
        if selected_option == "5":
            ...
            sku_categories = ["Raw Materials", "Consumer Goods",
            ...
            print(f"\nSaved! You have successfully added "
                  f"{inventory_item.sku} to {file_name}!\n")
            ...
            if not sku_list:
                ...
                while selected_choice not in choices:
                    selected_choice = input(f"Invalid Choice: Please enter "
                                            f"a valid number! (1 - {len(choices)}): ").strip()

                if selected_choice == "1":
                    print("-" * 56)
                    print("--- Select an Inventory Item "
                    ...
                    print(f"\nStock Keeping Unit: {sku_to_remove} has been "
                          f"successfully removed from inventory!\n")

                elif selected_choice == "2":
                    clear_inventory(file_name)
                    print(f"\nYou have permanently removed all "
                          f"Inventory Items from {file_name}!\n")
                else:
                    print(f"\nError: Something went wrong "
                          f"with, {selected_choice}")

        elif selected_option == "3":
            sku_list = list_skus(file_name)
            if not sku_list:
                print("\nThere are currently no saved Stock Keeping Units"
                      "(SKUs) to be listed!\n")
            else:
                print("-" * 64)
                print(f"--- All individually saved "
                      f"Stock Keeping Units from {file_name}")
                for index, item in enumerate(sku_list, 1):
                    print(f"{index}. {item[0]}, {item[1]}, {item[2]}")
                print("-" * 64)
                print()
            ...
            if not sku_list:
                print("\nThere are currently no saved "
                      "SKUs to be summarized!\n")
            else:
                print("-" * 56)
                print(f"--- The Quantity in Stock "
                      f"by Category from {file_name}:")
